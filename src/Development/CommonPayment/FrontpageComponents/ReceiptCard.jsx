...
const ReceiptCard = ({
  ...
  const downloadReceiptPDF = () => {
    ...
    pdf.setFont("helvetica", "normal");
    const planDetailLines = pdf.splitTextToSize(
      planDetails.replace(/<\/?[^>]+(>|$)/g, ""),
      130
    ); // Adjust the width for B5
  ...
  return (
    <div id="receipt" className="receipt-card relative" style={cardStyle}>
        ...
        <div>
          <ul>
            {subscriptionIds.length > 0 ? (
              subscriptionIds.map((id, index) => <li key={index}>{id}</li>)
            ) : (
      ...
      <button
        id="nextButton"
        className="w-full p-2 text-white mt-8"
      ...
      <button
        id="downloadButton"
        className="w-full p-2 text-white mt-4 flex gap-3 justify-center text-[1.1rem]"
